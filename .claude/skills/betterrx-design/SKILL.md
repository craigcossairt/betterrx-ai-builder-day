...
Production UI imports from `src/ui` (`Button`, `Input`, `Toast`, plus census/chrome/order/patient screens). Tokens are CSS variables. The eRX product look is white cards on `--surface-100`, blue `--blue-500` actions, coral accents, system UI font. Marketing look is Poppins, coral gradient pills, and navy ink.

Brand assets live in `public/brand/` (`logo-black.svg`, PWA icons, `pod-sample.svg`).
